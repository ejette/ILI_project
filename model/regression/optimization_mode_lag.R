...
# change the working directory to the path where you keep the ILI_project folder
setwd("~/Dropbox")
# function that lags the data
source("ILI_project/functions/make_positive_lag.R")
# function does the forward selection
...
# optimal lag data
load("ILI_project/data/regression/opt_lag.Rda")

# load death timeseries
load("ILI_project/data/regression/deaths_flu_seasons.Rda")
deaths = deaths_flu_seasons
# change column names to match the opt_lag data
colnames(deaths) = c('date','national',paste('region',1:10,sep=''))

# load ILINet data
load("ILI_project/data/regression/ili_flu_seasons.Rda")

# function find lags associated with mode lag for a given region
find_lag = function(lag_data, regions){
  for (i in 1:length(regions)){
    lag_freqs = count(lag_data$max_r2_lag[lag_data$region == regions[i]])
    mode_lag = which.max(lag_freqs$freq) - 1
    if (i == 1){
      opt_lag_data = data.frame(region = regions[i], type = 'mode', lag = mode_lag, stringsAsFactors = FALSE)
    } else {
...
# get a unique list of the region
regions = unique(opt_lag$region)
opt_lag_data = find_lag(opt_lag, regions)
  
# function lags the input data by the number of weeks specified by lag_step
...
# limit possible providers for which the colsum > 0
provider_data = ili_flu_seasons[,colSums(ili_flu_seasons) > 0]

# make 6 dataframes, each one using a different lag (0 through 5)
...
save(r2_values, file = 'ILI_project/data/regression_results/r2_values_mode_lag.Rda')

# the name variable will serve as a label for each of the columns in the ranks and r^2 dataframes
opt_lag_data$name = paste(opt_lag_data$region, opt_lag_data$type, opt_lag_data$lag, 'no_season', sep='_')

# run the optimization for each of the regions and at the national level
# record the results in ranks_mode_lag and r2_values_mode_lag
for (j in 1:nrow(opt_lag_data)){
  # set the candidate provider dataframe for the optimization to be the one mode lag for the target region
  ...
  load('ILI_project/data/regression_results/r2_values_mode_lag.Rda')
  # run the optimization
  ranks_mode_lag = var_select_cnty(obj = deaths[,opt_lag_data[j,'region']], vars = providers_lag[,2:(ncol(providers_lag))], goal = n_counties, r2_values = r2_values, ranks = ranks_mode_lag, type = 'mode')
  # name the last column in the ranks_mode_lag dataframe with the label in the opt_lag_data
  colnames(ranks_mode_lag)[length(colnames(ranks_mode_lag))] = opt_lag_data$name[j]
  save(ranks_mode_lag,file = 'ILI_project/data/regression_results/ranks_mode_lag.Rda')
  # name the last column in the r2_values_mode_lag dataframe
  load('ILI_project/data/regression_results/r2_values_mode_lag.Rda')
...
pdf(file = 'ILI_project/visualizations/regression_residuals_mode_lag.pdf', width = 8.5, height = 11)
par(mfrow=c(3,2))
for (i in 2:ncol(ranks_mode_lag)){ 
  # get the lag for the region
  lag = as.numeric(strsplit(colnames(ranks_mode_lag)[i], split = "_")[[1]][3])
  # split the colname to extract the information for the regression
  var_name = strsplit(colnames(ranks_mode_lag)[i], split = "_")
  type = paste(var_name[[1]][4],var_name[[1]][5],sep='_')
  # region_name is the formatted region name
  region_name  = var_name[[1]][1]
  region_name = gsub('region', 'HHS region ', region_name)
  # region is how it exists in the dependent variable dataframe
  region = var_name[[1]][1]
  # extract the top n providers for a region and format them
  top_providers = paste('total.',ranks_mode_lag[,i],sep='')
  # set the input dataframe to have the appropriate lag
  if (lag == 0){
    providers_lag = providers_lag0 
  }
  if (lag == 1){
  ...
  # make the predictor dataframe with the top providers
  X = providers_lag[,c(top_providers)]
  # bind predictors with death for region i
...
# r^2 plots
r2_mode_lag = r2_values
# make the data long for ggplot
r2_long = melt(r2_mode_lag, id.vars = 'index')
# make the variable more readable
r2_long$variable = substr(r2_long$variable,1,8)
r2_long$variable =gsub("region","",r2_long$variable)
r2_long$variable =gsub("_","",r2_long$variable)
...
pdf(file = 'ILI_project/visualizations/actual_vs_predicted_mode_lag.pdf', onefile = TRUE, width = 8.5, height = 11)
i = 2
