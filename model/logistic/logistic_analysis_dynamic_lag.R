...
# load y data
load("ILI_project/data/logistic/y_lag_dynamic.Rda")
y = y_lag_dynamic
...
coefs_dynamic = as.data.frame(t(coef(summary(fit))[,'Estimate']))
coefs_dynamic = cbind(as.character(colnames(y)[3]),coefs_dynamic)
colnames(coefs_dynamic) = c('city','intercept','hum','pop','per65', 'region_in', 'region_adjacent', 
                    'seasonal_reporting_rate', 'overall_reporting_rate',
                    'providers_per_FIPS','average_denom')
p_vals_dynamic = as.data.frame(t(coef(summary(fit))[,'Pr(>|z|)']))
p_vals_dynamic =  cbind(as.character(colnames(y)[3]), p_vals_dynamic)
colnames(p_vals_dynamic) = c('city','intercept','hum','pop', 'per65', 'region_in', 'region_adjacent', 
                     'seasonal_reporting_rate', 'overall_reporting_rate',
                     'providers_per_FIPS', 'average_denom')
sd_err_dynamic = as.data.frame(t(coef(summary(fit))[,'Std. Error']))
sd_err_dynamic =  cbind(as.character(colnames(y)[3]), sd_err_dynamic)
colnames(sd_err_dynamic) = c('city','intercept','hum','pop', 'per65', 'region_in', 'region_adjacent', 
                     'seasonal_reporting_rate', 'overall_reporting_rate',
                     'providers_per_FIPS','average_denom')
...
for (i in 3:(ncol(y)-3)){
  ...
  fit = glm(formula = y ~ hum + pop + per65 + region + seasonal_reporting_rate + overall_reporting_rate +
              providers_per_FIPS + average_denom, family = binomial(logit),  data = reg_df)
  coefs_dynamic_temp = cbind(as.character(colnames(y)[i+1]),t(coef(summary(fit))[,'Estimate']))
  colnames(coefs_dynamic_temp) = c('city','intercept','hum','pop', 'per65', 'region_in', 'region_adjacent', 
                           'seasonal_reporting_rate', 'overall_reporting_rate',
                           'providers_per_FIPS', 'average_denom')
  coefs_dynamic = rbind(coefs_dynamic, coefs_dynamic_temp)
  p_vals_dynamic_temp = cbind(as.character(colnames(y)[i+1]),t(coef(summary(fit))[,'Pr(>|z|)']))
  colnames(p_vals_dynamic_temp) = c('city','intercept','hum','pop', 'per65', 'region_in', 'region_adjacent', 
                            'seasonal_reporting_rate', 'overall_reporting_rate',
                            'providers_per_FIPS', 'average_denom')
  p_vals_dynamic = rbind(p_vals_dynamic,p_vals_dynamic_temp)
  sd_err_dynamic_temp = cbind(as.character(colnames(y)[i+1]),t(coef(summary(fit))[,'Std. Error']))
  colnames(sd_err_dynamic_temp) = c('city','intercept','hum','pop', 'per65', 'region_in', 'region_adjacent', 
                            'seasonal_reporting_rate', 'overall_reporting_rate',
                            'providers_per_FIPS','average_denom')
  sd_err_dynamic = rbind(sd_err_dynamic, sd_err_dynamic_temp)
  print(summary(fit))
}

p_vals_dynamic_region = p_vals_dynamic

# make a heatmap of p-values
mat_data <- data.matrix(p_vals_dynamic[,2:ncol(p_vals_dynamic)])
...
#pdf(file = "ILI_project/visualizations/heatmap_p_vals_dynamic_lag.pdf", width = 10, height = 10)
heatmap.2(mat_data,
