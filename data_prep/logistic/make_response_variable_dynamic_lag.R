...
colnames(y) = colnames(ranks_dynamic_lag[2])

# repeat for the rest of the regions
for (i in 3:ncol(ranks_dynamic_lag)){
  new_cols = c(colnames(y),colnames(ranks_dynamic_lag[i]))
  x = ranks_dynamic_lag[,i]
  x = substr(x,1,5)
...
y_lag_dynamic = y
colnames(y_lag_dynamic)[1] = 'FIPS_code'
save(y_lag_dynamic, file = 'ILI_project/data/logistic/y_lag_dynamic.Rda')
